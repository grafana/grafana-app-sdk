...
func TestUntypedKind_Read(t *testing.T) {
	...
	tests := []struct {
		...
		err:   fmt.Errorf("error reading kind: %w", emptyJSONErr),
	}, {
		name:  "missing APIVersion",
		input: []byte(`{"kind":"Foo","metadata":{}}`),
		err:   fmt.Errorf("error reading apiVersion: %w", emptyJSONErr),
	}, {
		name:  "missing Metadata",
		input: []byte(`{"kind":"Foo","apiVersion":"foo.bar/v1"}`),
		err:   fmt.Errorf("error reading metadata: %w", emptyJSONErr),
	}, {
		name:  "mostly empty",
