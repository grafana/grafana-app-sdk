...
import (
    ...
    "github.com/grafana/issue-tracker-project/pkg/plugin"
)

const (
    pluginID = "issuetrackerproject"
)
...
type PluginService struct { 
    issueService plugin.IssueService
}

// GetIssueService returns a IssueService
func (s *PluginService) GetIssueService(ctx context.Context) (plugin.IssueService, error) {
    return s.issueService, nil
}
...
func newInstanceFactory(logger logging.Logger) app.InstanceFactoryFunc {
	return func(ctx context.Context, settings backend.AppInstanceSettings) (instancemgmt.Instance, error) {
        ...
        if err != nil {
            logger.Error("failed to create Issue store", "err", err)
            return nil, fmt.Errorf("failed to create Issue store: %w", err)
        }
        svc.issueService = issueStore
