...
// CacheReflector watches a specified resource and causes all changes to be reflected in the given store.
type CacheReflector struct {
	...
	watchErrorHandler func(ctx context.Context, r *CacheReflector, err error)
...
func (r *CacheReflector) Name() string {
	return r.name
}

// TypeDescription returns the type description of the reflector.
func (r *CacheReflector) TypeDescription() string {
...
func (r *CacheReflector) Run(stopCh <-chan struct{}) {
...
func (r *CacheReflector) RunWithContext(ctx context.Context) {
	...
	go func() {
		...
		for {
			select {
			case <-t.C:
				logger.Info("cache.Reflector active listers", "num", r.activeListers.Load())
			case <-ctx.Done():
				return
			}
		}
...
func (r *CacheReflector) ListAndWatch(stopCh <-chan struct{}) error {
...
func (r *CacheReflector) ListAndWatchWithContext(ctx context.Context) error {
	logger := klog.FromContext(ctx)
...
func (r *CacheReflector) LastSyncResourceVersion() string {
...
func (r *CacheReflector) list(ctx context.Context) error {
...
func (r *CacheReflector) watch(ctx context.Context, w watch.Interface, resyncerrc chan error) error {
	stopCh := ctx.Done()
	logger := klog.FromContext(ctx)
	var err error
...
// It replaces its internal store with the collected items and
// reuses the current watch requests for getting further events.
func (r *CacheReflector) watchList(ctx context.Context) (watch.Interface, error) {
	stopCh := ctx.Done()
	logger := klog.FromContext(ctx)
	var w watch.Interface
	var err error
...
func (r *CacheReflector) watchWithResync(ctx context.Context, w watch.Interface) error {
...
func (r *CacheReflector) startResync(ctx context.Context, resyncerrc chan error) {
	logger := klog.FromContext(ctx)
...
func (r *CacheReflector) syncWith(items []runtime.Object, resourceVersion string) error {
...
func (r *CacheReflector) resyncChan() (<-chan time.Time, func() bool) {
	if r.resyncPeriod == 0 {
...
func (r *CacheReflector) setLastSyncResourceVersion(v string) {
...
func (r *CacheReflector) relistResourceVersion() string {
	r.lastSyncResourceVersionMutex.RLock()
	defer r.lastSyncResourceVersionMutex.RUnlock()
...
func (r *CacheReflector) rewatchResourceVersion() string {
	r.lastSyncResourceVersionMutex.RLock()
	defer r.lastSyncResourceVersionMutex.RUnlock()
	if r.isLastSyncResourceVersionUnavailable {
...
func (r *CacheReflector) setIsLastSyncResourceVersionUnavailable(isUnavailable bool) {
	r.lastSyncResourceVersionMutex.Lock()
	defer r.lastSyncResourceVersionMutex.Unlock()
