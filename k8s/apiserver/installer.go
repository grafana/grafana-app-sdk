...
// AppInstallerOptions contains customizable options for the AppInstaller generated by NewDefaultAppInstaller
type AppInstallerOptions struct {
	// StoreOptionsProvider returns StoreOptions for a Kind, or nil to use defaults provided by DefaultStoreOptions().
	StoreOptionsProvider func(kind resource.Kind) *StoreOptions
}

// StoreOptions is the set of storage options for a kind, allowing per-kind customization of storage.
type StoreOptions struct {
	// NameGenerator is used for generating names when `metadata.generateName` is specified.
	// It defaults to names.SimpleNameGenerator.
	ObjectNameGenerator names.NameGenerator
}

// DefaultStoreOptions returns a StoreOptions with defaults populated.
func DefaultStoreOptions() StoreOptions {
	return StoreOptions{
		ObjectNameGenerator: names.SimpleNameGenerator,
	}
}
...
//
//nolint:revive
func NewDefaultAppInstaller(appProvider app.Provider, appConfig app.Config, resolver GoTypeResolver, opts AppInstallerOptions) (*defaultInstaller, error) {
...
func (r *defaultInstaller) InstallAPIs(server GenericAPIServer, optsGetter genericregistry.RESTOptionsGetter) error {
	...
	for gv, kinds := range kindsByGV {
		...
		for _, kind := range kinds {
			...
			if r.options.StoreOptionsProvider != nil {
				if o := r.options.StoreOptionsProvider(kind.Kind); o != nil {
					if o.ObjectNameGenerator != nil {
						storeOpts.ObjectNameGenerator = o.ObjectNameGenerator
					}
				}
			}
			s, err := newGenericStoreForKind(r.scheme, kind.Kind, optsGetter, storeOpts)
			if err != nil {
