...
manifest: {
	...
	operator: {
		url: "https://myapp.svc.cluster.local:6443"
	}
}
```

> The `operator.url` is used to construct the conversion webhook configuration. If your operator serves the conversion webhook on a non-default path, set `operator.webhooks.conversionPath` (it defaults to `/convert`). See [App Manifest](../app-manifest.md#operator-and-webhook-paths) for details.
