...
* kind TypeScript code will be written to the path in `config.codegen.tsGenPath`, with a folder for each unique kind-version combination. Each app version additionally gets `<group>/<version>/api_gen.ts`: an [RTK Query](https://redux-toolkit.js.org/rtk-query/overview) API in the same shape as the clients core Grafana generates in `@grafana/api-clients`, with `use<Verb><Kind>Query`/`Mutation` hooks for list/get/create/replace/update/delete/deletecollection and each subresource of every kind in that version, plus a hook per custom route (kind and version level) with generated request/response types next to it. The shared `createBaseQuery.gen.ts` is written once at the root of `tsGenPath`. These use `getBackendSrv()` and `config.namespace` from `@grafana/runtime` and require `@reduxjs/toolkit` (and `react-redux` for the hooks) in the plugin.
