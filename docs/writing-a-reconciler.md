...
* If you run multiple replicas of the same operator and need only one replica to handle a given object, add a `simple.ShardFilter` in `simple.BasicReconcileOptions.ShardFilter` rather than duplicating ownership logic inside the reconciler itself. See [Event Sharding](event-sharding.md).
