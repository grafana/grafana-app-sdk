...
* Don't rely on retries to track operator state; use the `status` subresource to track operator success/failure, so that your operator can work out state from a fresh start (a restart will remove all pending retries, which are stored purely in-memory). This also allows a user to track operator status by viewing the `status` subresource. For runtime validation errors (e.g., external system checks, dynamic state validation), populate `fieldErrors` in the status. See [Field Errors in Status](./architecture/field-errors-in-status.md) for the recommended convention.
