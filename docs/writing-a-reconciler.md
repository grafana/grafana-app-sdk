...
* If your reconcile process makes requests for other resources, consider caching, as high-traffic objects may cause your application to have to make these requests extremely frequently. For typed resource update helpers like `UpdateObject[T]` and other advanced patterns, see [Advanced Operator Features](./advanced-operator-features.md).
