...
func (c *CodecGenerator) Generate(kind codegen.Kind) (codejen.Files, error) {
	...
	if c.OnlyUseCurrentVersion {
		...
		files = append(files, codejen.File{
			Data:         formatted,
			RelativePath: fmt.Sprintf("%s/%s_codec_gen.go", meta.Group, meta.MachineName),
		...
		for _, ver := range kind.Versions() {
			...
			files = append(files, codejen.File{
				Data:         formatted,
				RelativePath: fmt.Sprintf("%s/%s/%s_codec_gen.go", meta.Group, ToPackageName(ver.Version), meta.MachineName),
				From:         []codejen.NamedJenny{c},
			})
		}
	}
