...
func (j TypeScriptTypes) generateFiles(version *codegen.KindVersion, name, pathPrefix, prefix string) (codejen.Files, error) {
    ...
    tsBytes, err := generateTypescriptBytes(version.Schema, ToPackageName(version.Version), exportField(sanitizeLabelString(name)), j.GenBuilders, cog.TypescriptConfig{
...
func (j TypeScriptTypes) generateFilesAtDepth(v cue.Value, kv *codegen.KindVersion, currDepth int, pathPrefix string, prefix string) (codejen.Files, error) {
    if currDepth == j.Depth {
        ...
        tsBytes, err := generateTypescriptBytes(v, ToPackageName(kv.Version), exportField(strings.Join(fieldName, "")), j.GenBuilders, cog.TypescriptConfig{
...
func generateTypescriptBytes(v cue.Value, packageName string, name string, genBuilders bool, tsConfig cog.TypescriptConfig) ([]byte, error) {
